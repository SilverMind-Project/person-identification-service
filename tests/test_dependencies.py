...
def test_insightface_compatibility():
    ...
    major, minor, *rest = skimage_version.split('.')
    
    assert int(major) == 0, f"scikit-image major version {major} is not compatible with InsightFace 0.7.3"
    assert int(minor) < 26, f"scikit-image version {skimage_version} yields FutureWarnings in InsightFace 0.7.3. Must be < 0.26"
